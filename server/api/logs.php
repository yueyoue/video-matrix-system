...
if (function_exists('success')) {
    ...
    if (strpos($_SERVER['REQUEST_URI'], '/export') !== false) {
        ...
        $st = $pdo->query('SELECT * FROM ' . table('operation_log') . ' ORDER BY id DESC LIMIT 1000');
        while ($r = $st->fetch()) {
    ...
    $st = $pdo->prepare('SELECT COUNT(*) FROM ' . table('operation_log') . " WHERE $w");
    $st->execute($p); $tot = $st->fetchColumn();
    $st = $pdo->prepare('SELECT * FROM ' . table('operation_log') . " WHERE $w ORDER BY id DESC LIMIT $ps OFFSET " . ($page - 1) * $ps);
    $st->execute($p);
    success(['list' => $st->fetchAll(), 'total' => (int)$tot, 'page' => $page, 'pageSize' => $ps, 'pages' => ceil($tot / $ps)]);
    exit;
}
